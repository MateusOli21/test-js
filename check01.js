...
LoadCheckoutPaymentContext(function (Checkout, PaymentOptions) {
  ...
  const transparentCardPaymentOption = PaymentOptions.Transparent.CardPayment({
    ...
    fields: fillCardRequiredFields(Checkout.getData("country")),
...
function addChoProPaymentMethods() {
  const choPro = document.querySelector(
    `#radio-option-${CHECKOUT_PRO_PAYMENT_ID}`
  )
  const radioContent = document.querySelector(
    '.radio-content.payment-option-content'
  )

  if (!choPro && !radioContent) return

  const categoriesContainer = document.querySelector(
    '.payment-categories-container'
  )
  const base = `
  <div class="payment-categories-block">
    <div class="payment-categories-row">
      <div>
        <img class="payment-icon" alt="credit_card" src="/img/icons/payments/external/credit_card.svg">
      </div>
      <div class="payment-category-label text-semi-bold">
        Tarjeta de crédito
      </div>
    </div>
    <div class="payment-brands-general-container">
      <div class="payment-brands-polish" id="payment-brands-container-credit_card">
        <img alt="visa" src="/img/brands/visa.svg">
        <img alt="mastercard" src="/img/brands/mastercard.svg">
        <img alt="amex" src="/img/brands/amex.svg">
        </div>
      </div>
    </div>
  `
  categoriesContainer.innerHTML = base
}

function appendChoProInstallments(Checkout) {
  let chopro = document.querySelector('#radio-option-mercado_pago_checkout_pro')
  if (!chopro) return
  let choproHeader = chopro.querySelector('.accordion-section-header-label')
  if (!choproHeader) return
  let installmentsLabel = choproHeader.querySelector('.payment-item-discount')
  if (installmentsLabel) return

  const checkoutCard = Checkout.getData(TRANSPARENT_CARD_PAYMENT_ID)
  if (!checkoutCard) return
  const installmentsWithoutInterest =
    checkoutCard.supportedPaymentMethods.credit_card
      .max_installments_without_interest

  let div = document.createElement('div')
  div.className = 'payment-item-discount text-small-extra'
  let span = document.createElement('span')
  let text =
    Checkout.getData('country') == 'BR'
      ? `Até ${installmentsWithoutInterest} parcelas sem juros`
      : `Hasta ${installmentsWithoutInterest} meses sin interés`
  span.appendChild(document.createTextNode(text))
  div.appendChild(span)
  choproHeader.appendChild(div)
}

function overwriteInstallments(Checkout) {
    const checkoutCard = Checkout.getData(TRANSPARENT_CARD_PAYMENT_ID)
  if (checkoutCard) {
    const installmentsWithoutInterest =
      checkoutCard.supportedPaymentMethods.credit_card
...
function fillCardRequiredFields(country) {
  const requiredFieldsByCountry = {
      AR: { 
  ...
  const fields = requiredFieldsByCountry[country];
  return fields
}
