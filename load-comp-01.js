...
function setAnchorPoints() {
  ...
  let productSelector = document.querySelector('div[data-store="product-price"]');
  ...
  if (productBuyButtonSelector !== null)
    productAnchorElement = productBuyButtonSelector;
}
...
function loadMpWebComponent() {
  ...
  fetch(
    `https://api.mercadopago.com/beta/platforms/tienda-nube/payment/widgets/${LS.store.id}`
  )
    ...
    .then((data) => {
      ...
      if (addToCartButtonElement !== null) {
        ...
        if (data.product && LS.product) {
          console.log("Adicionar widget de produto");
          productAnchorElement.insertAdjacentHTML("beforeend", widgetHTML);
          productAnchorElement.appendChild(widgetScript);
        }

        if (data.cart && LS.product) {
          console.log("Adicionar widget do carrinho");
          cartAnchorElement.insertAdjacentHTML("beforeend", widgetHTML);
